...
class TrainLoop:
    ...
    def run_loop(self):
        def maybe_delete_all_freq():
            freq_files = [f for f in os.listdir(get_blob_logdir()) if f.startswith("freq_")]
            for f in freq_files:
            ...
            for batch, cond, _ in self.data:
                ...
                if self.step >= self.total_training_steps:
                    ...
                    if self.step  % self.save_interval != 0:
                        self.save()
                    print('Training completed', self.step)
                    break
            maybe_delete_all_freq()
            print('Erase freq files')
            condition = False
