...
from datasets.image_folder import make_dataset, make_test_dataset
...
from datasets.npzdataset import NPZDataset
import wandb
import time
...
def main():
    ...
    wandb.init(project='BridgePure-Sampling', name=f'{args.exp}', config=args)
    ...
    sample_dir = Path(workdir)/f'sample_{step}/w={args.guidance}_churn={args.churn_step_ratio}_{args.split}_{args.steps}'
    if os.path.exists(os.path.join(sample_dir, 'data_targets.npz')):
        print(f"Sample already exists in {sample_dir}")
        return
    ...
    if args.split =='train':
        ...
        paths = make_test_dataset(os.path.join(args.data_dir, 'val'))
        ids = [int(path.split('/')[-1].split('.')[0]) for path in paths]
        data_dir = get_data_dir(args.dataset)
        if args.dataset == 'cifar10':
            tmp_dataset = CIFAR10(root=data_dir, train=True, download=True, transform=None)
            targets = np.array(tmp_dataset.targets)
            class_names = tmp_dataset.classes
        elif args.dataset == 'cifar100':
            tmp_dataset = CIFAR100(root=data_dir, train=True, download=True, transform=None)
            targets = np.array(tmp_dataset.targets)
            class_names = tmp_dataset.classes
        elif 'webface' in args.dataset or 'imagenet' in args.dataset :
            tmp_dataset = NPZDataset(data_dir, train=True)
            targets = tmp_dataset.targets
            class_names = [str(idx) for idx in range(np.unique(targets).max())]
        else:
    ...
    for i, data in tqdm(enumerate(dataloader), total=len(dataloader), desc="sampling", leave=False):
        x0_image = data[0]
        ...
        wandb.log({'sampling_time': end - start, 'bath_size': len(y0), 'batch': i})
        ...
        if i == 0 and dist.get_rank() == 0:
            ...
            if class_names is not None and targets is not None:
                class_name_sample = [class_names[t] for t in targets[:num_display]]
                with open(f'{sample_dir}/sample_categories.txt', 'w') as f:
                    for category in class_name_sample:
                        f.write(category + '\n')
    ...
    if dist.get_rank() == 0:
        ...
        if args.split == 'train':
            out_path = os.path.join(sample_dir, f"data.npz")
            np.savez(out_path, data=arr)
        elif args.split == 'test':
            out_path = os.path.join(sample_dir, f"data_targets.npz")
            if targets is not None:
                np.savez(out_path, data=arr, targets=targets)
            else:
                np.savez(out_path, data=arr)
        else:
            raise NotImplementedError
        logger.log(f"shape: {shape_str}, saving to {out_path}")
